...
class SparkExportBean (@BeanProperty var sqlBean: SparkExportDmlBean) extends Serializable{
  ...
  def loopExec(sc: SparkContext,kafkaBrokerPath:String, jedisClusterYamlPath:String, month:String) {
    var i: Long = 0
    val loop = new Breaks;
    loop.breakable {
      while (true) {
        ...
        if (df != null && df.rdd != null && (!df.rdd.isEmpty())) {
          object InternalKafkaCli extends SparkKafkaHandler {}
          InternalKafkaCli.instance.send2Kafka(df, kafkaBrokerPath, jedisClusterYamlPath, sqlBean.getPartionNum, sqlBean.getEsIndex, sqlBean.getEsType, sqlBean.getDateColumnNames)
           LOG.info("kafka has send msgs bwtween "+i+","+endNo)
           i = endNo
        } else {
          loop.break
        }
