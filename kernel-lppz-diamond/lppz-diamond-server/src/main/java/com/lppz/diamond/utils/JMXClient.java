package com.lppz.diamond.utils;
...
public class JMXClient {
	...
	private static Map<String, JMXClient> cache = Collections
			.synchronizedMap(new HashMap<String, JMXClient>());
	...
	public static JMXClient getJMXClient(String hostName, int port,
			String userName, String password) throws JMXClientException {
		...
		log.debug("=============referenceCount:"
				+ jmxClient.referenceCount.get());
		return jmxClient;
	}
	...
	private JMXClient(String hostName, int port, String userName,
			String password) throws JMXClientException {
		this.hostName = hostName;
		this.port = port;
		this.userName = userName;
		this.password = password;
		try {
			this.address = new JMXServiceURL("service:jmx:rmi:///jndi/rmi://"
					+ this.hostName + ":" + this.port + "/jmxrmi");
	...
	public Object invoke(ObjectName name, String operationName,
			Object[] params, String[] signature) throws JMXClientException {
		tryReconnect();
		try {
			return this.mbs.invoke(name, operationName, params, signature);
		} catch (Exception e) {
			throw new JMXClientException("error occurred when invoke "
					+ name.getClass() + "#" + operationName, e);
		}
	}
	...
	public Object getAttribute(ObjectName name, String attribute)
		...
		try {
			...
			try {
				return this.mbs.getAttribute(this.queryMBeanForOne(name)
	...
	public void close() {
		...
		if (closed.compareAndSet(false, true)) {
			...
			cache.remove(getKey(this.hostName, this.port, this.userName,
					this.password));
	...
	private static String getKey(String hostName, int port, String userName,
			String password) {
