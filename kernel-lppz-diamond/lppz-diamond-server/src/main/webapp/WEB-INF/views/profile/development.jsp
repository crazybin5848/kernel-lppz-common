...
<div class="pull-right">
	...
	<a type="button" id="export" onclick="exportConfig()" href='javascript:void(0)' target="_self" class="btn btn-primary">导出</a>
</div>
<div id="uploadDiv" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-header">
    	<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
    	<h3 id="myModalLabel">上传配置</h3>
  	</div>
  	<div class="modal-body">
		<form id="importForm" method="post"  enctype="multipart/form-data" action="<c:url value='/file/fileUpload' />">
			<input type="file" id="uploadfile" name="uploadfile">
			<input type="hidden" name="projectId" value='<c:out value="${projectId}"/>'>
		</form>
	</div>
	<div class="modal-footer">
    	<button class="btn" data-dismiss="modal" id="closeUpload" aria-hidden="true">关闭</button>
    		...
    		<tr id='row-<c:out value="${config.CONFIG_ID}"/>'>
               	...
               	<td>
               		<c:if test="${project.OWNER_ID == sessionScope.sessionUser.id}">
                  		<a class="deleteConfig" href='/superdiamond/config/delete/<c:out value="${config.CONFIG_ID}"/>?projectId=<c:out value="${projectId}"/>&type=<c:out value="${type}"/>&moduleName=<c:out value="${config.MODULE_NAME}"/>&page=<c:out value="${currentPage}"/>&moduleId=<c:out value="${moduleId}"/>' title="删除"><i class="icon-remove"></i></a>
                  	</c:if>
...
$(document).ready(function () {
	...
	$("#preview").click(function(e) {
		window.location.href = '/superdiamond/profile/preview/<c:out value="${project.PROJ_CODE}"/>/<c:out value="${type}"/>/<c:out value="${moduleId}"/>?projectId=<c:out value="${projectId}"/>';
	});
...
function exportConfig(){
	var moduleId = '<c:out value="${moduleId}"/>'
		...
		alert(moduleId);
		window.location.href='<c:out value="/superdiamond/file/download/${project.PROJ_CODE}/"/>'+moduleId; 
	}
</script>