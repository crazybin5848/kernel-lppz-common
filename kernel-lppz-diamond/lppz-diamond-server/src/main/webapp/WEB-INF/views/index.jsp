  	...
  	<tbody>
    	<c:forEach items="${projects}" var="project">
       		<tr>
               	...
               	<td>
               		<c:if test="${sessionScope.sessionUser.userCode == 'admin'}">
               			<a href='./deployment/development/<c:out value="${project.id}"/>'>development</a>&nbsp;
               			<a href='./deployment/test/<c:out value="${project.id}"/>'>test</a>&nbsp;
               			<a href='./deployment/build/<c:out value="${project.id}"/>'>build</a>&nbsp;
               			<a href='./deployment/production/<c:out value="${project.id}"/>'>production</a>&nbsp;
               		</c:if>
               		<c:if test="${sessionScope.sessionUser.userCode != 'admin'}">
	               		<c:forEach items="${project.roles}" var="role">
	               			<c:if test="${role == 'development'}"> 
