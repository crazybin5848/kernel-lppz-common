...
public class IncrSequenceTimeHandler implements SequenceHandler {
	...
	private Properties loadProps(String propsFile){
		...
		File f=new File("configGen/"+SEQUENCE_DB_PROPS);
		try {
			InputStream inp = f.exists()?new FileInputStream(f):Thread.currentThread().getContextClassLoader().getResourceAsStream(propsFile);
			if (inp == null) {
