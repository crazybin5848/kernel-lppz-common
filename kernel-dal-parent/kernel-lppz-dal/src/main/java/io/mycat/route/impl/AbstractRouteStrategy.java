...
public abstract class AbstractRouteStrategy implements RouteStrategy {
     ...
     */
		...
		if (sc != null ) {
			...
			rrs.setTransNo(sc.getSession2().getTransNo());
		}
		...
		if(ServerParse.DELETE==sqlType&&!origSQL.toLowerCase().contains("where")){
//			MySqlDeleteStatement deleteStmt = (MySqlDeleteStatement) (new MySqlStatementParser(
//					origSQL)).parseDeleteStatement();
			String tablename = stmt.split(" ")[2];
			TableConfig tableConfig=schema.getTables().get(tablename.toUpperCase());
			Map<String,Set<String>> pkShardMap=MysqlDmlUtil.fetchIds4MultiKeyUpdateDel(rrs, tableConfig, null);
		...
		if(rrs.getSqlType() == ServerParse.UPDATE&&!origSQL.toLowerCase().contains("where")){
			MySqlUpdateStatement updateStmt = (MySqlUpdateStatement) (new MySqlStatementParser(
					origSQL)).parseUpdateStatement();
			String tablename = stmt.split(" ")[1];
			TableConfig tableConfig=schema.getTables().get(tablename.toUpperCase());
			Map<String,Set<String>> pkShardMap=MysqlDmlUtil.fetchIds4MultiKeyUpdateDel(rrs, tableConfig, null);
			for(String shardValue:pkShardMap.keySet()){
				Set<String> pkSet=pkShardMap.get(shardValue);
