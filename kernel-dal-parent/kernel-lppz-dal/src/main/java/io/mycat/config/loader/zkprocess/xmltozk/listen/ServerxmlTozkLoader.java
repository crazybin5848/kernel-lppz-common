...
public class ServerxmlTozkLoader extends ZkMultLoader implements NotiflyService {
    ...
    */
    ...
    public ServerxmlTozkLoader(ZookeeperProcessListen zookeeperListen, CuratorFramework curator,
            XmlProcessBase xmlParseBase, boolean ... isAddListen) {
      ...
      //启动服务时，创建临时节点会用这个构造方法创建对象获取server对象，加载监听会覆盖zktoxml监听，
      //导致运行时同步zk server数据变成上传本地数据，造成配置同步无效，这里加一个可变参数区分处理
        if (isAddListen.length == 0) {
        	// 将当前自己注册为事件接收对象
    ...
    public Object notiflyProcess(boolean write) throws Exception {
        // 1,读取本地的xml文件
        Server server = this.parseServerXMl.parseXmlToBean(write?SERVER_PATH:"configGen/server.xml");
