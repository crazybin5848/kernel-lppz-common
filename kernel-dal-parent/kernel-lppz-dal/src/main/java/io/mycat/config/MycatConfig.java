...
public class MycatConfig {
	...
	public MycatConfig(Map<String, PhysicalDBPool> dataHostMap,Map<String, PhysicalDBNode> dataNodeMap) {
		//读取schema.xml，rule.xml和server.xml
		boolean mark=true;
		if(dataHostMap!=null){
			mark=false;
			this.dataHosts = dataHostMap;
			this.dataNodes = dataNodeMap;
		}
		ConfigInitializer confInit = new ConfigInitializer(mark);
