...
public class XMLSchemaLoader implements SchemaLoader {
	...
	public XMLSchemaLoader(String schemaFile, String ruleFile) {
		...
		//读取加载schema配置
		this.load(DEFAULT_DTD, !new File(schemaFile).exists() ? DEFAULT_XML : schemaFile);
	}

	public XMLSchemaLoader() {
		this("configGen/schema.xml", "configGen/rule.xml");
	}
	...
	private void load(String dtdFile, String xmlFile) {
		...
		try {
			...
			xml = f.exists()?new FileInputStream(f):XMLSchemaLoader.class.getResourceAsStream(xmlFile);
	...
	private void loadSchemas(Element root) throws SQLSyntaxErrorException {
	...
	private Map<String, TableConfig> loadTables(Element node) throws SQLSyntaxErrorException {
		...
		for (int i = 0; i < nodeList.getLength(); i++) {
			...
			for (int j = 0; j < tableNames.length; j++) {
				...
				String rootParentKey = tableElement.hasAttribute("rootParentKey") ? tableElement.getAttribute("rootParentKey").toUpperCase() : null;
				String secondaryIndexKeys = tableElement.hasAttribute("secondaryIndexKeys") ? tableElement.getAttribute("secondaryIndexKeys").toUpperCase() : null;
				...
				TableConfig table = new TableConfig(tableName, primaryKey,
						...
						ruleRequired, null, false, null, null,subTables,rootParentKey,build(secondaryIndexKeys,primaryKey,null,rootParentKey,false));
	...
	private Set<String> build(String secondaryIndexKeys, String primaryKey, String joinKey, String rootParentKey, boolean isChildTable) throws SQLSyntaxErrorException {
		Set<String> setString=new TreeSet<String>();
		if(StringUtils.isNotBlank(primaryKey)&&isChildTable){
			setString.add(primaryKey.toLowerCase());
			if(StringUtils.isNotBlank(joinKey)){
				setString.add(joinKey.toLowerCase());
			}
			if(StringUtils.isNotBlank(rootParentKey)){
				setString.add(rootParentKey.toLowerCase());
			}
		}
			
		if(StringUtils.isNotBlank(secondaryIndexKeys)){
			if(secondaryIndexKeys.contains("-"))
			  throw new SQLSyntaxErrorException("secondaryIndexKeys can not contains:-!");
			for(String s:secondaryIndexKeys.split("::")){
				setString.add(s.toLowerCase());
			}
		}
		return setString;
	}
	...
	private void processChildTables(Map<String, TableConfig> tables,
			TableConfig parentTable, String dataNodes, Element tableNode) throws SQLSyntaxErrorException {
		...
		for (int j = 0; j < childNodeList.getLength(); j++) {
			...
			String rootParentKey = childTbElement.hasAttribute("rootParentKey") ? childTbElement.getAttribute("rootParentKey").toUpperCase() : null;
			String secondaryIndexKeys = childTbElement.hasAttribute("secondaryIndexKeys") ? childTbElement.getAttribute("secondaryIndexKeys").toUpperCase() : null;
			...
			TableConfig table = new TableConfig(cdTbName, primaryKey,
					...
					joinKey, parentKey, subTables,rootParentKey,build(secondaryIndexKeys,primaryKey,joinKey,rootParentKey,true));
	...
	private DBHostConfig createDBHostConf(String dataHost, Element node,
			String dbType, String dbDriver, int maxCon, int minCon, String filters, long logTime, String autoCommit, String connectionTimeout, String dataSourceClassName, String idleTimeout, String maxLifetime) {
		...
		conf.setAutoCommit(Boolean.parseBoolean(autoCommit));
		conf.setConnectionTimeout(Long.parseLong(connectionTimeout));
		conf.setDataSourceClassName(dataSourceClassName);
		conf.setIdleTimeout(Long.parseLong(idleTimeout));
		conf.setMaxLifetime(Long.parseLong(maxLifetime));
		return conf;
	}
	...
	private void loadDataHosts(Element root) {
		...
		for (int i = 0, n = list.getLength(); i < n; ++i) {
			...
			String dataSourceClassName = element.getAttribute("dataSourceClassName");
			String idleTimeout = element.getAttribute("idleTimeout");
			String maxLifetime = element.getAttribute("maxLifetime");
			for (int w = 0; w < writeDbConfs.length; w++) {
				Element writeNode = (Element) writeNodes.item(w);
				writeDbConfs[w] = createDBHostConf(name, writeNode, dbType, dbDriver, maxCon, minCon,filters,logTime,autoCommit,connectionTimeout,dataSourceClassName,idleTimeout,maxLifetime);
				...
				if (readNodes.getLength() != 0) {
					...
					for (int r = 0; r < readDbConfs.length; r++) {
						...
						readDbConfs[r] = createDBHostConf(name, readNode, dbType, dbDriver, maxCon, minCon,filters, logTime,autoCommit,connectionTimeout,dataSourceClassName,idleTimeout,maxLifetime);
					}
