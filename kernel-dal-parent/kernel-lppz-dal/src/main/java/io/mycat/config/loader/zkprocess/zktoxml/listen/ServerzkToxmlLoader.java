    ...
    */
    ...
    public Object notiflyProcess(boolean write) throws Exception {
        ...
        this.getTreeDirectory(currZkPath, ZookeeperPath.FLOW_ZK_PATH_SERVER.getKey(), serverDirectory);
        	...
        	if(CollectionUtils.isEmpty(serverDirectory.getSubordinateInfo()))
        		return false;
        // 从当前的下一级开始进行遍历,获得到
...
//                .getPath();
        ...
        String path = "configGen/" + WRITEPATH;
    ...
    */
    private Server zktoServerBean(DiretoryInf zkDirectory) {
        ...
        DataInf serverZkDirectory=null;
        // 得到server对象的目录信息
        if(StringUtils.isBlank(myid))
        	serverZkDirectory = this.getZkData(zkDirectory, ZookeeperPath.FLOW_ZK_PATH_SERVER_DEFAULT.getKey());
        else{
        		ZkDirectoryImpl zkClusterDir=(ZkDirectoryImpl)zkDirectory.getSubordinateInfo().get(0);
            List<Object> ll=zkClusterDir.getSubordinateInfo();
            for(Object o:ll){
            		ZkDataImpl zkNode=(ZkDataImpl)o;
            		if(myid.equals(zkNode.getName())){
            			serverZkDirectory = zkNode;
            			break;
            		}
            }
        }
