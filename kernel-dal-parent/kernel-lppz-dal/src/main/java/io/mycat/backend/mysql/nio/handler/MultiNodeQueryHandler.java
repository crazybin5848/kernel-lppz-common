...
public class MultiNodeQueryHandler extends MultiNodeHandler implements
	...
	public MultiNodeQueryHandler(int sqlType, RouteResultset rrs,
		...
		this.rrs = rrs;
		isOffHeapuseOffHeapForMerge = MycatServer.getInstance().getConfig()
	...
	public void execute() throws Exception {
		...
		final MycatConfig conf = MycatServer.getInstance().getConfig();
		startTime = System.currentTimeMillis();
		LOGGER.debug("rrs.getRunOnSlave()-" + rrs.getRunOnSlave());
		final boolean isLocked = session.getSource().isLocked();
		final AtomicInteger ai = new AtomicInteger(0);
		boolean isAllLoadDDL = true;
		for (final RouteResultsetNode node : rrs.getNodes()) {
			if (node.getLoadData() == null&&checkDDL(node)) {
				isAllLoadDDL = false;
				break;
			}
		}
		if (isAllLoadDDL) {
			int size = rrs.getNodes().length;
			for (final RouteResultsetNode node : rrs.getNodes()) {
				new Thread(new Runnable() {
					@Override
					public void run() {
						try {
							doexec(conf, isLocked, node);
						} catch (Exception e) {
	...
	private boolean checkDDL(final RouteResultsetNode node) {
		boolean isNoTruncate = node.getStatement()!=null&&!node.getStatement().toLowerCase().startsWith("truncate");
		boolean isNoDrop = node.getStatement()!=null&&!node.getStatement().toLowerCase().startsWith("drop");
		boolean isNoAlter = node.getStatement()!=null&&!node.getStatement().toLowerCase().startsWith("alter");
		return isNoTruncate&&isNoDrop&&isNoAlter;
	}

	private void doexec(MycatConfig conf, boolean isLocked,
			final RouteResultsetNode node) throws Exception {
	...
	public void okResponse(byte[] data, BackendConnection conn) {
		...
		if (executeResponse) {
			...
			boolean isCanClose2Client = (!rrs.isCallStatement())
					|| (rrs.isCallStatement() && !rrs.getProcedure()
							.isResultSimpleValue());
			;
			if (!isCallProcedure) {
				if (clearIfSessionClosed(session)) {
	...
	public void rowEofResponse(final byte[] eof, BackendConnection conn) {
		...
		if (execCount == rrs.getNodes().length) {
			int resultSize = source.getWriteQueue().size()
					* MycatServer.getInstance().getConfig().getSystem()
							.getBufferPoolPageSize();
			// TODO: add by zhuam
			// 查询结果派发
			QueryResult queryResult = new QueryResult(session.getSource()
					.getUser(), rrs.getSqlType(), rrs.getStatement(),
	...
	public void outputMergeResult(final ServerConnection source,
			final byte[] eof, Iterator<UnsafeRow> iter) {
		try {
			lock.lock();
			...
			/**
			 * 处理limit语句的start 和 end位置，将正确的结果发送给 Mycat 客户端
			 */
		...
		try {
			...
			if (prepared) {
				for (int i = start; i < end; i++) {
					...
					binRowDataPk.packetId = ++packetId;
					// binRowDataPk.write(source);
					buffer = binRowDataPk.write(buffer, session.getSource(),
							true);
				}
			} else {
				for (int i = start; i < end; i++) {
	...
	public void rowResponse(final byte[] row, final BackendConnection conn) {
		...
		try {
			...
			if (dataMergeSvr != null) {
				...
				// close the connection for tx control such as rollback or
				// commit.
