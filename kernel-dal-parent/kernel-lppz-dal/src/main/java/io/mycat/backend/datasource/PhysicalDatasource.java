...
import io.mycat.backend.jdbc.HikariJDBCDatasource;
...
//		int ildeCloseCount = hostConfig.getMinCon() * 3;
	...
	private BackendConnection takeCon(BackendConnection conn,
		...
		if(conn instanceof JDBCConnection){
			((JDBCConnection)conn).setRunning(true);
		}
...
//		JDBCConnection jconn=null;
//		if(con instanceof JDBCConnection)
//			jconn=(JDBCConnection)con;
//		if(jconn!=null&&(jconn.getPool() instanceof HikariJDBCDatasource)){
//			createNewConnection(handler, attachment, schema);
//			LOGGER.info("HikariJDBC Conn doing");
//			return;
//		}
		if (con != null/*&&(jconn==null||!(jconn.getPool() instanceof HikariJDBCDatasource))*/) {
			...
			if (activeCons + 1 > size) {// 下一个连接大于最大连接数
				...
				LOGGER.debug("no ilde connection in pool,create new connection for "	+ this.name + " of schema " + schema);
				createNewConnection(handler, attachment, schema);
			}
		}
	}
	...
	private void returnCon(BackendConnection c) {
		...
		if(c instanceof JDBCConnection){
			((JDBCConnection)c).setRunning(false);
		}
